...
1. **Collection Phase**
   - Python files scanned by AST analyzer
   - System metrics sampled via psutil
   - Shell scripts capture OS-level data

2. **Processing Phase**
   - Complexity scores calculated
   - Thresholds compared against baselines
   - Metrics aggregated into unified format

3. **Output Phase**
   - Charts generated from time-series data
   - Reports exported in multiple formats
   - CI status updated based on thresholds
